...
def comparate_messages (message, candidate_message):
  ...
  if message.text and candidate_message.text:
    ...
    if similarity < ACCEPTANCE:
      ...
      print("Mensagem do usuário:", message_text, ", mensagem candidata:", candidate_text, ", confiança:", similarity)
  
  return similarity

def select_response(message, list_response, storage.None):
  response = list_response[0]
  print("response escolhida:", response)

  return response

def execute_bot():
  bot = ChatBot("Robô de consultas do Campeonato Brasileiro",
  read_only = True,
